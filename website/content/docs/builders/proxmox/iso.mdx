...
- `http_ipv6` - (bool) - Specifies whether Packer should use the IPv6 address
  for the `HTTPIP`. Defaults to false.

- `vm_interface` - (object) - Network interface used when Packer determines the
  VMs IP address. Example:
  ```json
  {
    "interface": "eth0",
    "ipv6": true
  ...
  ```
  - `interface`: - (string) - Name of the network interface that Packer gets
    the VMs IP from. Defaults to the first non loopback interface.
  - `ipv6`: - (bool) - Specifies whether Packer should use the IPv6 address of
    an interface. Defaults to false.
